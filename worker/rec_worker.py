...
def install(app_module):
    ...
    def process_match(match_index, data, match):
        # Native .rec is preferred. Keep the existing MP4/video processor as a fallback.
        ...
        while len(matches) < 7: matches.append({})
        match = dict(matches[match_index])
        match["status"] = "Parsing .REC replay..."
        matches[match_index] = match
        app_module.db.collection("tournaments").document("oregano-stats").set({"matches": matches}, merge=True)
        ...
        try:
            ...
            # Remove this match's previous contribution before replacing it.
            ...
            for p in players:
                name = p["name"]
                target = totals.setdefault(name, {"name": name})
                contribution = {}
                for key in ("kills","deaths","assists","headshots","plants","defusals","rounds","clutches","entryKills","entryDeaths","reinforcements","abilityImpact","gadgetUses","supportActions","gameWinningMoves"):
                    contribution[key] = float(p.get(key, 0) or 0)
                    target[key] = float(target.get(key, 0) or 0) + contribution[key]
